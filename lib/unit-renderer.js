...
UnitRenderer.prototype.renderMovementAndAttackOptions = function() {
  ...
  for (var j = 0; j < this.state.board.tiles.length; j++) {
    ...
    if(this.unit.canAttackTile(tile.loc) && (tile.exists || this.unit.currentAttack().type === 'add_tile')) {
      this.rendering.attackOptions.push(addSpriteAndConvert(tile.loc, 'attack-option'));
    }
    var dist = Tile.dist(tile.loc, this.unit.head);
    if(!this.unit.attackMode && !arrayEqual(tile.loc, this.unit.head) && dist <= this.unit.movesRemaining() && tile.exists) {
      if(dist === 1) {
        this.rendering.movementOptions.push(addSpriteAndConvert(tile.loc, 'movement-option'));
      } else {
        this.rendering.movementOptions.push(addSpriteAndConvert(tile.loc, 'movement-option-2'));
      }
    }
  }
};
